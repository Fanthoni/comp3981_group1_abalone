...
class SettingsMenu:
    ...
    def pack_starting_positions_options(self):
        ...
        for pos in [positions for positions in StartingPositions if positions is not StartingPositions.EMPTY]:
            opt = tk.Radiobutton(
                frame, text=pos.value, value=pos, variable=self._starting_pos
            )
            opt.pack()
    ...
    def pack_player1_mode_options(self):
        ...
        for player_mode in [mode for mode in PlayerMode]:
            opt = tk.Radiobutton(
                frame, text=player_mode.value, value=player_mode, variable=self._player1_mode
            )
            opt.pack()
    ...
    def pack_player2_mode_options(self):
        ...
        for player_mode in [mode for mode in PlayerMode]:
            opt = tk.Radiobutton(
                frame, text=player_mode.value, value=player_mode, variable=self._player2_mode
            )
            opt.pack()
